...
local group
local states = {}
local admin = "steam:11000010ce3b619"
states.frozen = false
...
RegisterNUICallback('quick', function(data, cb)
	if data.type == "slay_all" 
	or data.type == "bring_all" 
	or data.type == "slap_all" 
	or data.type == "heal_all" 
	or data.type == "status_all"
	or data.type == "revive_all" then
...
AddEventHandler('es_admin:quick', function(t, target,name)
...
AddEventHandler('reviveallplayer', function(name)
	local ped = GetPlayerPed(-1)
	if GetEntityHealth(ped) == 0 then
		TriggerEvent('samild:revive')
	end
...
RegisterNetEvent('samild:revive')
AddEventHandler('samild:revive', function()
