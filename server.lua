...
TriggerEvent("es:addGroup", "mod", "superadmin", function(group) end)
...
AddEventHandler('es_admin:all', function(type)
	...
	TriggerEvent('es:canGroupTarget', user.getGroup(), "admin", function(available)
			...
			if available or user.getGroup() == "superadmin" then
				...
				if type == "heal_all" then TriggerClientEvent('es_admin:quick', -1, 'heal')print( 'Heal by '..xPlayer.name..'') end
				if type == "status_all" then TriggerClientEvent('es_admin:quick', -1, 'status_all') print( 'Water and Eat by '..xPlayer.name..'')end
				if type == "revive_all" then TriggerClientEvent('es_admin:quick', -1, 'revive_all') print( 'Reviveall by '..xPlayer.name..'') end
			else
				DropPlayer(source, "คุณถูกแบน")
			end
		end)
	end)
end)
			...
			TriggerEvent('es:canGroupTarget', user.getGroup(), groupsRequired[type], function(available)
				...
				TriggerEvent('es:canGroupTarget', user.getGroup(), target.getGroup(), function(canTarget)
					if canTarget and available then
						...
						if type == "status" then TriggerClientEvent('es_admin:quick', id, type) end
						...
						if type == "slay" then TriggerClientEvent('es_admin:quick', id, type) end
						if type == "kick" then DropPlayer(id, 'ติดต่อห้องแอดมิน') end
					
						if type == "ban" then
							for k,v in ipairs(GetPlayerIdentifiers(id))do
								banUser(v)
							end
							DropPlayer(id, GetConvar("es_admin_banreason", "คุณถุกแบน"))
						end
					else
...
-- Kicking
TriggerEvent('es:addGroupCommand', 'kick', "superadmin", function(source, args, user)
	if args[1] then
...
-- Announcing
TriggerEvent('es:addGroupCommand', 'announce', "superadmin", function(source, args, user)
...
-- Bring
local frozen = {}
TriggerEvent('es:addGroupCommand', 'bring', "superadmin", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
...
-- Freezing
local frozen = {}
TriggerEvent('es:addGroupCommand', 'goto', "superadmin", function(source, args, user)
	if args[1] then
		if(GetPlayerName(tonumber(args[1])))then
			local player = tonumber(args[1])
